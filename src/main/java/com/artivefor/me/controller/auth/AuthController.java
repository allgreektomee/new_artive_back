...
public class AuthController {
    ...
    // 1. 회원가입 (추가됨)
    @PostMapping("/signup")
    public ApiResponse<Void> signUp(@RequestBody AuthRequest.SignUp request) {
        authService.signUp(request);
        return ApiResponse.success(MessageCode.AUTH_SIGNUP_SUCCESS);
    }

    // 2. 이메일 인증 번호 전송 (개선)
    @PostMapping("/email/send")
    public ApiResponse<Void> sendEmailCode(@RequestBody AuthRequest.EmailSend request) {
        authService.sendCode(request.getEmail());
        // MessageCode.Auth에 EMAIL_SEND_SUCCESS("auth.email.send.success") 추가 필요
        return ApiResponse.success(MessageCode.AUTH_EMAIL_SEND_SUCCESS);
    }

    // 3. 인증 번호 확인 (개선)
    @PostMapping("/email/verify")
    public ApiResponse<Void> verifyCode(@RequestBody AuthRequest.EmailVerify request) {
        authService.verifyCode(request.getEmail(), request.getCode());
        return ApiResponse.success(MessageCode.AUTH_EMAIL_VERIFY_SUCCESS);
    }
